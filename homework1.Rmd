...
```
It appears that gender has the strongest effect on length of stay, as its regression coefficient has the highest absolute value.  Assuming that length of stay is measured in days, on average males stay 4.51 days longer than females. 
```
...
```{r}
pt(2.91711, 7, lower.tail=FALSE)*2
```
...
```
The second test, where var.equal=TRUE is called, produces an identical p-value as is seen in the summary(mod) output.
```
